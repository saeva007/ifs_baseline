...
# ==========================================
# 0. Stage1 overlap 基线 — 实验根目录与 checkpoint 均在 ifs_baseline/ 下。
#    默认 S1 数据：build_s1_pm10_overlap_from_full.py 产出
#    ml_dataset_pmst_v5_aligned_12h_pm10_pm25_overlap（27 dyn + 36 FE）。
#    若改用 notebook 月末数据，可将 S1_DIR 改为 ml_dataset_s2_tianji_12h_pm10_pm25_monthtail_2
#    （需放在本目录下或写绝对路径）。
# ==========================================
TARGET_WINDOW_SIZE = 12
BASE_PATH = "/public/home/putianshu/vis_mlp/ifs_baseline"
S1_DIR = os.path.join(BASE_PATH, "ml_dataset_pmst_v5_aligned_12h_pm10_pm25_overlap")

CONFIG = {
    # ========== 实验控制 ==========
    'EXPERIMENT_ID':          os.environ.get('EXPERIMENT_JOB_ID', 'exp_s1_overlap_pm10_baseline'),
    'S1_PRETRAINED_PATH':     None,
    'BASE_PATH':              BASE_PATH,
    'WINDOW_SIZE':            TARGET_WINDOW_SIZE,
    'S1_DATA_DIR':            S1_DIR,
    # 与 train/s1.py 一致：可用 PMST_NUM_WORKERS=0 规避个别队列上 DataLoader 卡死。
    ...
    # - appended pm10 + pm2p5 as the last two dyn vars => +2
    'DYN_VARS_COUNT':          27,
    # NOTE:
...
class ImprovedDualStreamPMSTNet(nn.Module):
    def __init__(self, dyn_vars_count=25, window_size=12, static_cont_dim=5,
        ...
        # pm10 (index -2) and pm2p5 (index -1) feed the GRU together with met/zenith subset.
...
def _resolve_dyn_and_fe_dims(total_dim: int, win_size: int):
    ...
    在 dyn 候选 (27,26,25,24) 中选取使 FE 落在合理区间者（与 train/s1.py 一致）。
    """
...
def _dyn_indices_log1p(dyn_vars_count: int):
    """与 train/s1.py 一致：降水/辐射/CAPE + 气溶胶(末1～2 通道) 做 log1p。"""
    idxs = [2, 4, 9]
...
def load_data(data_dir, scaler=None, rank=0, local_rank=0, device=None,
    ...
    RobustScaler 仅在训练集行上拟合；不再对 X_train 随机切分验证（避免时间窗泄漏）。
    """
    ...
    if not os.path.isfile(path_X_va) or not os.path.isfile(path_y_va):
        raise FileNotFoundError(
            ...
            "Build S1 dataset with notebook so val split matches monthly+gap design."
        )
    ...
    scaler_path = os.path.join(
        ...
        f'robust_scaler_w{win_size}_dyn{dyn_vars_count}_s1_overlap_pm10.pkl'
    )
