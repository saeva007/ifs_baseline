...
CONFIG = {
    ...
    'S1_DATA_DIR':            S1_DIR,
    'S2_DATA_DIR':            S2_DIR,
    # [性能修复 1] 恢复使用环境变量设定，避免被硬编码 0 卡死
    'NUM_WORKERS':            int(os.environ.get('PMST_NUM_WORKERS', '2')),
...
# [性能修复 3] 在 GPU 上进行纯 Tensor 计算，移除 .any() 带来的 CPU 隐式同步阻塞
def compute_soft_targets(vis_raw, hard_labels, num_classes=3):
    ...
    alpha_fm = (vis[fm_mask] - 400) / 200.0
    soft[fm_mask, 0] = 1.0 - alpha_fm
    soft[fm_mask, 1] = alpha_fm
    soft[fm_mask, 2] = 0.0

    mc_mask = (vis >= 800) & (vis < 1200)
    alpha_mc = (vis[mc_mask] - 800) / 400.0
    soft[mc_mask, 0] = 0.0
    soft[mc_mask, 1] = 1.0 - alpha_mc
    soft[mc_mask, 2] = alpha_mc
...
def train_stage(tag, model, tr_ds, val_ds, optimizer, loss_fn, device,
    ...
    # [性能修复 1] 恢复与一阶段一致的非阻塞 DataLoader 设计
    num_workers = max(0, int(CONFIG.get('NUM_WORKERS', 2)))
    prefetch_factor = max(1, int(os.environ.get('PMST_PREFETCH_FACTOR', '2')))
    
    loader_kw = dict(batch_sampler=sampler, pin_memory=True, worker_init_fn=worker_init_fn)
    if num_workers > 0: 
        loader_kw.update(num_workers=num_workers, prefetch_factor=prefetch_factor, persistent_workers=True)
    else: 
    ...
    if num_workers > 0: 
        val_loader_kw.update(num_workers=num_workers, persistent_workers=True, prefetch_factor=prefetch_factor)
    else: 
    ...
    #[性能修复 2] 在主循环前预筛选 L2-SP 参数对，避免每次 Forward 都去寻址和查找 Hash
    l2sp_pairs =[]
    if pretrained_state is not None and l2sp_alpha > 0:
        ...
        for n, p in raw_m.named_parameters():
            if n in pretrained_state and p.requires_grad and p.shape == pretrained_state[n].shape:
                l2sp_pairs.append((p, pretrained_state[n]))
    ...
    while step < total_steps:
        ...
        # [性能修复 3] 将数据先转移到 GPU (non-blocking并发)，然后再在 device 上计算 soft_targets
        bx = bx.to(device, non_blocking=True)
        ...
        if CONFIG.get('USE_LABEL_SMOOTHING', False):
            soft_targets = compute_soft_targets(braw, by)
        ...
        # [性能修复 2] L2-SP 使用预先缓存的参数对计算，极大降低 Kernel Launch 开销
        if pretrained_state is not None and l2sp_alpha > 0 and l2sp_pairs:
            l2_sp = 0.0
            for p, ref_p in l2sp_pairs:
                l2_sp = l2_sp + torch.sum((p - ref_p) ** 2)
...
def main():
    ...
    def build_s2_optimizer_phase_a(raw_model):
        ...
        # [性能修复 4] Phase A 中没有需要多余检查的未使用参数了，关闭 find_unused
    ...
    val_loader_final = DataLoader(
        ...
        num_workers=max(0, int(CONFIG.get('NUM_WORKERS', 2))), pin_memory=True, worker_init_fn=_worker_init_fn,
    )
